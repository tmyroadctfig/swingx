 ...
 * <code>JTable.PRINT_MODE_NORMAL</code> is used, the implementation
 ...
 * When <code>JTable.PRINT_MODE_FIT_WIDTH</code> is given, the implementation
 ...
 * sizes works best when <code>JTable.PRINT_MODE_FIT_WIDTH</code> is used, or
 * when only the printable width is changed between pages. This is because when
 * it is printing a set of rows in <code>JTable.PRINT_MODE_NORMAL</code> and the
...
class TablePrintable implements Printable {
     ...
     *                       <code>JTable.PRINT_MODE_NORMAL</code> or
     *                       <code>JTable.PRINT_MODE_FIT_WIDTH</code>
    ...
    public TablePrintable(JTable table,
        ...
        if (printMode != JXTable.PRINT_MODE_NORMAL &&
                printMode != JXTable.PRINT_MODE_FIT_WIDTH) {

            throw new IllegalArgumentException("Unknown Print Mode: " +
                                               printMode);
        }
    ...
    private void findNextClip(int pw, int ph) {
        ...
        // we can short-circuit for JTable.PRINT_MODE_FIT_WIDTH since
