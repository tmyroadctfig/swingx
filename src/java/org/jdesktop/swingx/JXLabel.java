...
public class JXLabel extends JLabel {
     ...
     * By default this support is disabled to mimic default of the JLabel. 
    ...
    /**
     * Returns the current status of line wrap support. The default value of this property 
     * is false to mimic default JLabel behavior.
     * Value of this property has no effect on HTML text.
     * 
            ...
            if (src.isLineWrap()) {
                if ("font".equals(name) || "foreground".equals(name)) {
                    ...
                    if (isHTML((String)evt.getOldValue()) 
                            && evt.getNewValue() != null && !isHTML((String) evt.getNewValue())) {
                        // was html , but is not
...
//            getStyle("default").addAttributes(attr);
    ...
    static class Renderer extends AlterWrappedPlainView {
        ...
        Renderer(JComponent c, ViewFactory f, View v, boolean wordWrap) {
            super(null, wordWrap);
            factory = f;
            view = v;
            view.setParent(this);
     ...
     */
        ...
        private boolean isJustifiableDocument() {
            return (! Boolean.TRUE.equals(getDocument().getProperty(
                          AlterAbstractDocument.I18NProperty)));
        }
        ...
        protected void layoutMajorAxis(int targetSpan, int axis, 
            ...
            for (int i = getViewCount() - 1; i >= 0 ; i--) {
                ...
                if (view instanceof AlterGlyphView) {
                    AlterGlyphView.JustificationInfo justificationInfo = 
                        ((AlterGlyphView) view).getJustificationInfo(rowStartOffset);
    ...
    static class AlterUtilities extends Utilities {
        ...
        private static int drawChars(JComponent component, Graphics g, char[] txt,
                int flushIndex, int flushLen, int x, int y) {
            try {
                return (Integer) drawChars.invoke(null, new Object[] {component, g, txt,
...
//            return Toolkit.getDefaultToolkit().getFontMetrics(g.getFont());    }
        ...
        static int getNextVisualPositionFrom(View v, int pos, Position.Bias b,
            ...
            retValue = child.getNextVisualPositionFrom(-1, biasRet[0],
                ...
                if ((direction == SwingConstants.EAST ||
                     direction == SwingConstants.WEST) &&
                    (v instanceof AlterCompositeView) &&
                    ((AlterCompositeView)v).flipEastAndWestAtEnds(pos, b)) {
                    increment *= -1;
                }
    ...
    static class AlterWrappedPlainView extends AlterBoxView implements TabExpander {
        ...
        if (AlterUtilities.isComposedTextAttributeDefined(attr)) {
            ...
            x = AlterUtilities.drawComposedText(this, attr, g, x, y, 
        ...
        if (wordWrap) {
            p = p0 + AlterUtilities.getBreakLocation(segment, metrics,
                            tabBase, tabBase + currentWidth,
                            this, p0);
        } else {
            p = p0 + AlterUtilities.getTabbedTextOffset(segment, metrics, 
                               tabBase, tabBase + currentWidth,
                               this, p0, false);
        }
            SegmentCache.releaseSharedSegment(segment);
        ...
        class WrappedLine extends View {
            ...
            int testP = (b == Position.Bias.Forward) ? pos :
                ...
                if (pos > p0) {
                    ...
                    alloc.x += AlterUtilities.getTabbedTextWidth(segment, metrics, 
                            alloc.x, AlterWrappedPlainView.this, p0);
                    SegmentCache.releaseSharedSegment(segment);
                }
                return alloc;
            }
            ...
            alloc.height = metrics.getHeight();
                    ...
                    if (line >= lineCount) {
                        ...
                        if (x < alloc.x) {
                            ...
                            Segment segment = SegmentCache.getSharedSegment();
                            loadText(segment, p0, p1);
                            int n = AlterUtilities.getTabbedTextOffset(segment, metrics, 
                                                       alloc.x, x, 
